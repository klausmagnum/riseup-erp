/**
 * Dispara a captura de NFS-e de um cliente pela linha de comando.
 *
 * Serve para a primeira execução de um cliente, quando se quer olhar o
 * resultado antes de deixar o agendamento assumir. Grava no banco e arquiva
 * no Drive — não é um verificador, é a sincronização de verdade.
 *
 *   npx tsx scripts/sincronizar-nfse.mts --cnpj 49039801000150
 *   npx tsx scripts/sincronizar-nfse.mts --cnpj 49039801000150 --lotes 1
 */
...
} from "../app/lib/nfse/sincronizarClienteNFSe.ts";
...
// O arquivamento no Drive domina o tempo: 50 documentos levam cerca de 2min.
...
console.log(`NSU de partida: ${cliente.ultimo_nsu_nfse_recebida ?? 0}`);
console.log(`limite de lotes: ${maxLotes}\n`);
