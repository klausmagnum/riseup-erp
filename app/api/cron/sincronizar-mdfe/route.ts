...
} from "@/app/lib/sefaz/sincronizarClienteMDFe";
...
/**
 * Captura automática de MDF-e.
 *
 * Roda em horário próprio: os quatro fluxos usam o mesmo certificado do cliente,
...
// NF-e. O piso não é folga: sincronizarClienteMDFe reserva um lote inteiro
...
// vez, sem sequer falar com a SVRS.
...
export async function GET(request: Request) {
  ...
  const { data: clientes, error: filaError } = await supabase
    ...
    .eq("status", "Ativo")
    .eq("sincronizacao_mdfe_ativa", true)
    .or(`proxima_sincronizacao_mdfe.is.null,proxima_sincronizacao_mdfe.lte.${agora}`)
    .order("ultima_sincronizacao_mdfe", { ascending: true, nullsFirst: true })
    .limit(120)
