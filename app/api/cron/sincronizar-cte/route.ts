...
import {
  sincronizarClienteCTe,
  type ClienteSincronizavelCTe,
} from "@/app/lib/sefaz/sincronizarClienteCTe";
import type { ResultadoSincronizacao } from "@/app/lib/sefaz/sincronizarCliente";
 ...
 *
 * Roda em horário próprio: os três fluxos usam o mesmo certificado do cliente,
 * e tanto a SEFAZ quanto o ambiente nacional recusam consultas em sequência.
 */
...
export async function GET(request: Request) {
  ...
  const { data: clientes, error: filaError } = await supabase
    .from("clientes")
    .select("id,razao_social,identificacao,estado,ultimo_nsu_cte_recebida")
    .eq("status", "Ativo")
    .eq("sincronizacao_cte_ativa", true)
    .or(`proxima_sincronizacao_cte.is.null,proxima_sincronizacao_cte.lte.${agora}`)
    .order("ultima_sincronizacao_cte", { ascending: true, nullsFirst: true })
    .limit(120)
    .returns<ClienteSincronizavelCTe[]>();
  ...
  for (const cliente of clientes) {
    ...
    const resultado = await sincronizarClienteCTe({
      supabase,
      cliente,
      certificado,
      ambiente: "producao",
